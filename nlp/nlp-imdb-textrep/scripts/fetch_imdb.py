"""Download the IMDB train + test parquet files into data/imdb/.
...
doesn't match the pinned checksum is deleted rather than left half-trusted.
    ...
    python scripts/fetch_imdb.py [--data-dir data/imdb]
"""
import argparse
import hashlib
import os
...
from db_logic.loaders.imdb import IMDB_FILES, imdb_local_path, imdb_url  # noqa: E402
...
def fetch(data_dir: str) -> None:
    ...
    for split, (_, expected) in IMDB_FILES.items():
        dest = imdb_local_path(data_dir, split)
        if os.path.exists(dest) and sha256(dest) == expected:
            print(f"{split}: already present ({dest})")
            continue
        print(f"{split}: downloading {imdb_url(split)}")
        tmp = dest + ".part"
        urllib.request.urlretrieve(imdb_url(split), tmp)
        actual = sha256(tmp)
        if actual != expected:
            os.remove(tmp)
