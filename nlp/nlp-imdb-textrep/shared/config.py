...
class Config:
    ...
    def __init__(self):
        ...
        # IMDB parquet files (make data). Mounted read-only in the container;
        # no stated licence, so the data stays out of the image.
        self.IMDB_DATA_DIR = os.getenv("IMDB_DATA_DIR", "/app/data/imdb")
        # 0 means "every row" (25k train, 25k test).
        self.IMDB_MAX_TRAIN_ROWS = int(os.getenv("IMDB_MAX_TRAIN_ROWS", "0")) or None
        self.IMDB_MAX_TEST_ROWS = int(os.getenv("IMDB_MAX_TEST_ROWS", "0")) or None
