...
PyTorch CNN classifier for handwritten digits (MNIST 28×28 grayscale, 10 classes). Demo UI is an HTML5 canvas where visitors draw a digit and get a prediction with per-class confidence. First DL demo in the pandyaHomeLab platform; instantiated from `ml/_templates/ml-project-template/` (the template skeleton is domain-agnostic — same 3-layer structure works for ML, DL, NLP, Agentic).

Public URL: `https://pandyahomelab.com/dl/mnist-cnn/` (live after Phase 2a.7 integration).
Experiment tracking: `https://mlflow-dl.pandyahomelab.com/` (live now — Phase 2a.1b).
